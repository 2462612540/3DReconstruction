%% 学习目标:深入学习字符串%%字符串最基本的玩法一定要掌握clc;
%查看所有str的函数是 help strfun 
clear all;
str='I am daxian';
disp(str);                                                             %字符串显示
str_size=size(str);                                               %字符串的长度
str(1:7);                                                               %截取字符串的一部分显示
str2=str(end:-1: 15)                                           %字符串反序输出
...
n2=str1>=str2;                                                  %字符的比较
n3=ge(str1,str2);                                                 %采用函数ge字符的比较


str='I am dAxian';
n1=strfind(str, 'n');
n2=strfind('n', str);                                             %注意顺序
n3=strfind(str, 'am');                                           %查找字符串  找到的是的首字母的第一个位置
...
clear all;
str='I am daxian';
sl=strrep(str,'daxian','dashen');                          %字符串替换
s2=strrep(str,'Daxin','dashen');                          %如果没有找到，输出为原来的字符串

t1=num2str(randn (3,3));                                   %将数字转换为字符串
pi=num2str(pi,5);                                               %只显示6位
t2=int2str(randn(3,3));                                       %先取整，然后转换为字符串
x1=str2nun(t2);                                                  %将字符串矩阵转换为数字矩阵
